...
Future<TransactionsList> fetchPost() async {
  ...
  final response = await http.get(
      'https://asianbitcoins.org/abc/api/getcompletedtrades.php?key=anu5781&email=' +
          email);
...
class DashboardCompletedTrade extends StatefulWidget {
  static const sid = 'dashboard_completed_trade';

  @override
  _DashboardCompletedTradeState createState() =>
      _DashboardCompletedTradeState();
}

class _DashboardCompletedTradeState extends State<DashboardCompletedTrade> {
  String value;
...
//                                ),
          ...
          if (snapshot.data.transactions.length == 0) {
            list.add(Text(
              "You don’t have any closed trade.",
              textAlign: TextAlign.center,
            ));
          }
          return Container(
...
_launchURL() async {
  ...
  String email = prefs.getString('email');
  String url =
      'https://asianbitcoins.org/abc/api/completedtrades_transaction_uid.php?key=anu5781&email=' +
          email;
