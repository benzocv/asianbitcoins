...
Future<TransactionsList> fetchPost2(int page_no) async {
  ...
  final response = await http.get(
      'https://asianbitcoins.org/abc/api/sell_coin_dashboard.php?key=anu5781&email=' +
          email +
          '&otp=' +
          otp +
          '&page=' +
...
class SellCoinDashboard extends StatefulWidget {
  static const sid = 'sell_coin_dashboard';

  @override
  _SellCoinDashboardState createState() => _SellCoinDashboardState();
}

class _SellCoinDashboardState extends State<SellCoinDashboard> {
  String value;
                ...
                GestureDetector(
                  onTap: () {
                    if (snapshot.data.transactions[i].email != email) {
                      Navigator.pushNamed(context, SellTradeExplore.sid,
...
//
          ...
          list.add(Center(
              child:
                  Text("Please wait for the new advertisement to be posted")));
